    ...
    registry.register_passthrough_nullable_2_arg(
        ...
        vectorize_with_builder_2_arg::<VariantType, ArrayType<StringType>, BooleanType>(
            |v, keys, output, ctx| {
                ...
                match RawJsonb::new(v).exists_any_keys(keys.iter().map(|k| k.as_bytes())) {
                    Ok(res) => output.push(res),
                    Err(err) => {
                        ctx.set_error(output.len(), err.to_string());
                        output.push(false);
    ...
    registry.register_passthrough_nullable_2_arg(
        ...
        vectorize_with_builder_2_arg::<VariantType, ArrayType<StringType>, BooleanType>(
            |v, keys, output, ctx| {
                ...
                match RawJsonb::new(v).exists_all_keys(keys.iter().map(|k| k.as_bytes())) {
                    Ok(res) => output.push(res),
                    Err(err) => {
                        ctx.set_error(output.len(), err.to_string());
                        output.push(false);
    ...
    registry.register_passthrough_nullable_2_arg(
        ...
        vectorize_with_builder_2_arg::<VariantType, StringType, BooleanType>(
            |v, key, output, ctx| {
                ...
                match RawJsonb::new(v).exists_all_keys(once(key.as_bytes())) {
                    Ok(res) => output.push(res),
                    Err(err) => {
                        ctx.set_error(output.len(), err.to_string());
                        output.push(false);
